  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-white shadow-sm">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 bg-orange-50 px-2.5 py-1 rounded-full">
              ...
              <span className="text-orange-600" style={{ fontWeight: 700, fontSize: '0.75rem' }}>{state.user.streak}</span>
            </div>

            {/* Level + XP */}
            <div className="flex items-center gap-1.5">
              <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full" style={{ fontWeight: 700, fontSize: '0.7rem' }}>{level}</span>
        ...
        <nav className="hidden md:flex border-t border-slate-100 max-w-2xl mx-auto w-full">
          {navItems.map(item => (
            <NavLink
              key={item.to}
              to={item.to}
      ...
      {/* ── MOBILE BOTTOM NAV ── */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-100 shadow-[0_-1px_10px_rgba(0,0,0,0.06)]">
